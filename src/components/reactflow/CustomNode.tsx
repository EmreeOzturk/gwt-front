...
export default function CustomNode({
  ...
  //console.log("CustomNode",{userCount, users, show, address, xPos, yPos, data, id});
  const dispatch = useAppDispatch();
  const [loading, setLadng] = useState(false);
  const addChildNode: MouseEventHandler = async (evt) => {
    ...
    try {
      ...
      const itemChildren = await Promise.all([
        ...data.children.map(async (item: any, i: number) => {
          //console.log("item", item);
          console.log("children sayısı:", data.children.length);
          ...
          const datas: any = await Datas({ item, data, id, getNFTInfo });
          console.log("data.leftChild", data.leftChild, data.leftChild === 0);
          //console.log("item", item);
          ...
          return {
            ...
            data: {
              label: datas?.id,
              type: Number(data.leftChild) === 0 ? true : false,
      ...
      const newNodeId2 = `${id}__${new Date().getTime()}2`;

      // the edge between the clicked node and the child node is created
      itemChildren.reverse().map((item: any, i: number) => {
        addNodes(item);
        addEdges({
          id: `${id}->${item.id}`,
          source: id,
      ...
      id: newNodeId,
      position: { x: xPos, y: yPos + 100 },
      data: { label: 'R',index:1, type:true },
    });
    addNodes({
      id: newNodeId2,
      position: { x: xPos, y: yPos + 100 },
      data: { label: 'L',index:0 , type:false },
      
    });
    addEdges({ id: `${id}->${newNodeId}`, source: id, target: newNodeId,type: "smoothstep", });
    addEdges({ id: `${id}->${newNodeId2}`, source: id, target: newNodeId2,type: "smoothstep", }); */
    } catch (error) {
  ...
  return (
    <div
      onClick={!data.expandable ? addChildNode : () => {}}
      className="border-4 border-purple backdrop-blur-sm bg-white/10 text-white flex justify-center rounded-lg p-3 w-full relative z-20"
      style={{
        width: "250px",
        height: "120px",
        display: id === "A" ? "none" : "flex",
      }}
    >
      <div className="absolute top-1 text-2xl font-semibold rounded-full ">
        {!data?.first &&
          (data?.index === 0 && !data?.type ? (
      ...
      <div className="grid grid-cols-3 items-center  gap-0 pt-2 w-full">
        <svg
          className="w-10"
          viewBox="0 0 27 27"
          fill="none"
        ...
        <div className="col-span-2 flex flex-col items-end">
          <span>User {Number(data?.label) - 1}</span>
          <h4>
            {data?.address &&
        ...
        <div className="col-span-2 hidden gap-2 items-center justify-end opacity-80">
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
          ...
          User {Number(data?.refId) - 1 || 0}
        </div>

        {data?.expanded && data?.children?.length > 1 && !loading ? (
          <>
            <div className="border-[3px] absolute animate-fadeIn -bottom-14 w-[120px] text-center justify-center text-[10px] -left-10 flex items-center bg-[#D2F5DC] text-green-600 border-green-400 p-2 h-7 rounded-full shrink-0">
              Revenue: ${data?.leftChildRevenue || 0}
            </div>
            <div className="border-[3px] absolute animate-fadeIn -bottom-14 w-[120px] text-center justify-center text-[10px] -right-10 flex items-center bg-[#D2F5DC] text-green-600 border-green-400 p-2 h-7 rounded-full shrink-0">
              Revenue: ${data?.rightChildRevenue || 0}
            </div>
          </>
        ) : (
          data?.expanded &&
          data?.children?.length === 1 && !loading &&  (
            <>
              <div className="border-[3px] absolute animate-fadeIn -bottom-14 w-[120px] text-center justify-center text-[10px] left-1/4 flex items-center bg-[#D2F5DC] text-green-600 border-green-400 p-2 h-7 rounded-full shrink-0">
                Revenue: $
                {data?.leftChildRevenue || data?.rightChildRevenue || 0}
              </div>
            </>
          )
        )}
...
async function Datas({
  ...
  let datas = {
    ...
    name: "a",
    parentID: id + "." + contractData.id.toString(),
